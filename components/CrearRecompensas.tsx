...
export default function Recompensas() {
  ...
  async function createRecompensaFromInput() {
    ...
    try {
      client.queries
        .categorize({ prompt: recompensasArray })
        .then(async ({ data: categorizedData, errors }) => {
          if (errors)
            throw console.error(
            ...
            if (Array.isArray(newCategorizedTransacciones)) {
              newCategorizedTransacciones.map(({ text, category }) =>
                client.models.Recompensa.create({
                  ...
                  if (errors)
                    throw console.error("Error al crear la recompensa", errors);
  ...
  function handleRecompensaCategoriaChange(id: string, categoriaId: string) {
    console.log(
      `Actualizando recompensa ${id} con la categoría ${categoriaId}`
    );
          ...
          <li key={recompensa.id}>
            ...
            <div>
              <select
                ...
                onChange={(e) =>
                  handleRecompensaCategoriaChange(recompensa.id, e.target.value)
                }
            ...
            <button onClick={() => handleElminiarRecompensa(recompensa.id)}>
              Eliminar recompensa
            </button>
          </li>
        ))}
